...
- Product: **Model Builder 4.0**
- Parameter schema: **4**
- Platform: Windows
- Abaqus integration tested on: **Abaqus 2022**
- Final shareable package: `release\ModelBuilder_4.0\` or `release\ModelBuilder_4.0.zip`
...
1. Extract `ModelBuilder_4.0.zip` to a normal application folder. Do not run it from inside the ZIP.
...
The D40 preset is tagged **FVA-RESEARCH** and uses a locked internal geometry backend whose filename retains `v3` for traceability. The product and schema remain v4. The preset can create a research physics setup with contacts, steps, BCs, amplitude and one load cycle, while a strict guard prevents creation or submission of an Abaqus Job.
...
- Linear hexahedron choices: `C3D8` (default), `C3D8I`, or `C3D8R` (retained for reproducibility, with a warning where applicable).
- Quadratic meshes use the supported quadratic element strategy in the engine.
- Local notch partitions, arc seeding and graded bands are enabled by default; the audit records the real element mix, failed/warning elements, aspect ratios, connected components and fillet resolution.
- Robust repair may convert unsuitable cells to tetrahedra. Selecting a hex code does not promise a 100% hexahedral mesh.
- A build audit marked OK is not proof of stress convergence. Use a dedicated convergence study before reporting a stress concentration factor.
...
`project.json` records identity, configuration, run state and discovered artifacts. Absolute artifact routes are written into `input\params.json`; Abaqus runs with `jobs` as its working directory so solver files cannot leak beside the EXE.
...
3. `PARAM_BUILD_AUDIT.json.verdict` is `OK`, or every `CHECK` is understood and accepted by an engineer.
...
## Verified v4 integration example

A real end-to-end regression was run on 2026-09-10 with Abaqus 2022:

| Item | Verified value |
|---|---|
| Geometry | D40, form AB, N1, cylindrical hub |
| Lengths | nominal 40 mm; shaft slot 50 mm; bearing 34 mm |
| Mesh | linear C3D8 strategy; coarse global seeds 4/1.5/4 mm; 4 elements on notch arc |
| Result | 33,520 nodes; 35,851 elements; 75.5% hex; 0 failed elements; one connected component per part |
| Dimensions | shaft 40 × 40 × 120 mm, PASS |
| Artifacts | CAE, TXT/JSON audits, `BUILD_RESULT.ok=true`, four PNGs, manifest, report data and TeX |
| Solver | NOJOB; `jobs` remained empty |

The intentionally coarse mesh produced an aspect-ratio warning and is not a converged stress model. The regression proves automation and artifact routing, not structural adequacy. No LaTeX compiler was installed on that QA machine, so TeX generation was verified but PDF compilation was not.
...
`run_builder.bat` is a convenience launcher that prefers a packaged EXE and otherwise uses `py -3`. `build_headless.bat` is retained only for explicit legacy flat-output reproduction; it is not included in the shareable package and refuses to run unless `--legacy-flat` is supplied.
...
The script pins PyInstaller 6.22.2, builds outside the historical `dist` tree, runs the packaged `--self-test`, and invokes `make_release.py`. The release helper copies only this whitelist:
...
It rejects Abaqus databases, solver files, replay/journal files, run-specific audits and loose source files in the package. The self-test validates packaged resources and core defaults; it does not start Abaqus or prove an engineering analysis.
